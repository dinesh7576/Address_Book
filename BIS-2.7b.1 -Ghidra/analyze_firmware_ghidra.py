...
def check_control_flow_complexity():
    ...
    iterator = model.getCodeBlocks(TaskMonitor.DUMMY)
    count = 0
...
def save_results():
    ...
    for entry in results["high_entropy_regions"]:
        writer.writerow(["High Entropy", "", entry["address"], "Entropy: {}".format(entry["entropy"])])
...
log_console("Debug Symbols Present: {}".format(results["debug_symbols"]))
...
save_results()
log_console("JSON report: {}".format(json_path))
log_console("CSV report: {}".format(csv_path))
